        ...
        <div className="flex items-center justify-between py-4 gap-4 md:gap-8">
          ...
          <nav className="hidden md:flex justify-center">
            <ul className="flex items-center gap-6">
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-base font-medium text-textColor hover:text-hoverText px-2 py-1 rounded transition-colors duration-300">
                    {item.label}
                  </Link>
                </li>
              ))}
            ...
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  onClick={closeMenu}
                  className="block rounded-lg px-3 py-2 text-lg font-medium text-textColor hover:text-hoverText transition-colors duration-300"
                >
                  {item.label}
                </Link>
              </li>
